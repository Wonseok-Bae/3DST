...
void analyze(string file){
	...
	// Loop over all events===========================================================여기서부터 모든 이벤트에 대한 루프=========================
	for (int ievent = 0; ievent < nevents; ievent++){
		//cout<<"line 288 "<<"t_vtx[0] : "<<t_vtx[0]<<endl;
		...
		// only study event in Fiducial Volume========================================FV 안에서 일어나는 이벤트=================================
		// TODO: Change this according to the geometry
		if (abs(t_vtx[0]) < 50 && abs(t_vtx[1]) < 50 && abs(t_vtx[2]) < 50){
			...
			for (int ineutronHit = 0; ineutronHit < 100; ineutronHit++){
				...
				// TODO: Change this according to the geometry
				// cout<<"line 336 "<<"t_vtx[0] : "<<t_vtx[0]<<endl;

				if (abs(t_neutronHitX[ineutronHit]) < 120 && abs(t_neutronHitY[ineutronHit]) < 120 &&
						abs(t_neutronHitZ[ineutronHit]) < 100){
					if (ineutronHit == 0) cout<<"line 342 "<<"Neutrino vertex X Y Z : "<<t_vtx[0]<<"  "<<t_vtx[1]<<"  "<<t_vtx[2]<< "  Neutron vertex X Y Z : "<<t_neutronHitX[ineutronHit]<<"  "<<t_neutronHitY[ineutronHit]<<"  "<<t_neutronHitZ[ineutronHit]<<endl;		    
					...
					if (signalWindow > 0){
						...
						//cout<<"line 368 "<<"temp.vtxSignal[0] : "<<temp.vtxSignal[0]<<endl;
		...
		if ((outFV_inside3DST && NC) || out3DST){
			...
			for (int ineutronHit = 0; ineutronHit < 100; ineutronHit++){
				...
				if (abs(t_neutronHitX[ineutronHit]) < 120 && abs(t_neutronHitY[ineutronHit]) < 120 &&
					...
					//cout<<"line509 "<<"sig_earliestHit.vtxSignal[0] : "<<sig_earliestHit.vtxSignal[0]<<endl;
					...
					if (backgroundWindow > 0){
						...
						temp.vtxSignal[1] = t_vtx[1];
						temp.vtxSignal[2] = t_vtx[2];
						//cout<<"line 534 " <<"temp.vtxSignal[0] : "<<temp.vtxSignal[0]<<endl;
						string key = string(Form("%d_%d_%d", (int) t_neutronHitX[ineutronHit],
									(int) t_neutronHitY[ineutronHit],
									(int) t_neutronHitZ[ineutronHit]));
