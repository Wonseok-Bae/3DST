...
using namespace std; //소속

//1.2 필요한 거 달아놓기 - 히스토그램 선언===============================================================================================================================================================
// Histograms {{{
...
TH2F* cut_2D = new TH2F("cut_2D", "cut_2D;Lever Arm [cm]; Time [ns]", 20, 0, 200, 25, 0, 25);//컷 없이 래버암,TOF에 따른 시그널 그래프. (명칭, 제목;X축;Y축, 빈개수,X시작,X끝, 빈개수,Y시작,Y끝)
TH2F* nocut_2D = new TH2F("nocut_2D", "nocut_2D;Lever Arm [cm]; Time [ns]", 20, 0, 200, 25, 0, 25);//컷 적용해서 래버암,TOF에 따른 시그널 그래프. (명칭, 제목;X축;Y축, 빈개수,X시작,X끝, 빈개수,Y시작,Y끝)

TH1F * cut_arm0 = new TH1F("cut_arm0","cut_arm0",20,0,200);
TH1F * cut_arm1 = new TH1F("cut_arm1","cut_arm1",20,0,200);
TH1F * cut_arm2 = new TH1F("cut_arm2","cut_arm2",20,0,200);
TH1F * cut_arm3 = new TH1F("cut_arm3","cut_arm3",20,0,200);
TH1F * cut_arm4 = new TH1F("cut_arm4","cut_arm4",20,0,200);
TH1F * cut_arm5 = new TH1F("cut_arm5","cut_arm5",20,0,200);
TH1F * cut_arm6 = new TH1F("cut_arm6","cut_arm6",20,0,200);
TH1F * cut_arm7 = new TH1F("cut_arm7","cut_arm7",20,0,200);
TH1F * cut_arm8 = new TH1F("cut_arm8","cut_arm8",20,0,200);
TH1F * cut_arm9 = new TH1F("cut_arm9","cut_arm9",20,0,200);
TH1F * cut_arm10 = new TH1F("cut_arm10","cut_arm10",20,0,200);
...
TH1F * threshold_vs_efficiency = new TH1F("test","test",11,0,0.000011);//X-문턱값 Y-이피션시 그래프 x축 범위, (명칭, 제목, 빈 개수, X시작 값, X끝값)
TH2F* leverarm_vs_time_efficiency = new TH2F("efficiency", "efficiency;Lever Arm [cm]; Time [ns]", 20, 0, 200, 25, 0, 25);//컷 없이 래버암,TOF에 따른 이피션시 그래프

//TH2F* cut_efficiency = new TH2F("cut_efficiency", "cut_efficiency;Lever Arm [cm]; Time [ns]", 20, 0, 200, 25, 0, 25);//컷 없이 래버암,TOF에 따른 이피션시 그래프
//TH2F* nocut_efficiency = new TH2F("nocut_efficiency", "nocut_efficiency;Lever Arm [cm]; Time [ns]", 20, 0, 200, 25, 0, 25);//컷 적용해서 래버암,TOF에 따른 이피션시 그래프
// }}}
//1.3 필요한 거 달아놓기 - 변수생성===============================================================================================================================================================
// count
long int bkg = 0,
...
class Statistics_t //이건 쓴 적 없는 것 같은데, 클래스 변수 
{
    private:
        int n;
...
std::vector<Hit_t> sig_Neutron_Lvr[20], sig_Neutron_Time[25]; // Hit_t 구조체로 변수 찍어내기
std::vector<Hit_t> bkg_Neutron_Lvr[20], bkg_Neutron_Time[25]; // Hit_t 구조체로 변수 찍어내기

// Testing variables
Statistics_t  sig_Stats_Lvr[20], bkg_Stats_Lvr[20]; // Statistics_t 클래스로 변수 찍어내기
std::vector<float> sig_Value_Lvr[20], bkg_Value_Lvr[20], sig_Value_Time[25], bkg_Value_Time[25]; //Vector타입 float정밀도 변수 
map<string, int> countCubeHit; //뭔지 모르는 변수 맵, 스트링? 인트 정밀도?
Statistics_t sig_Stats_TrueE[30]; // Statistics_t 클래스로 변수 찍어내기
//}}}

//2.1 에너지 운동에너지 구하는 공식 ========================================================================================================================================================================
// Function {{{
float recEnergy(Float_t l, Float_t t)//에너지 리컨스트럭션
{
    return pow((723 * l)/t, 2) * 0.000000001;
}

float kineticEnergy(Float_t px, Float_t py, Float_t pz, Float_t e)//운동 에너지 리컨스트럭션
{
...
// }}}
//========================================================================================================================================================================================================
void analyze(string file)//애널라이즈는루트 데이터 파일을 가져다 읽는 것. 
{
    auto _file = new TFile(TString(file));//file이라는  이름을 가진 루트 데이터 파일을 파일 변수에 연결
    auto tree = (TTree*)_file->Get("tree");//그 파일에서 트리-데이터형식-을 뽑아서 연결
    ...
    int nevents = tree->GetEntries();//파일 안에 있는 데이터들을 이 코드에 있는 변수들에 연결
            ...
            for (int ineutronHit = 0; ineutronHit < 100; ineutronHit++)
            {
                ...
                if (abs(t_neutronHitX[ineutronHit]) < 120 && abs(t_neutronHitY[ineutronHit]) < 120 && abs(t_neutronHitZ[ineutronHit]) < 100)
                {
                    ...
                    if (signalWindow > 0)
                    {
                        ...
                        // Raise signal flag//실질적으로 analyze하는 부분
                        is_Sig = true;
                        nocut_2D->Fill(hitPerCube.at(key).trackLength,hitPerCube.at(key).timeWindow);//nocut_2D에 그래프 채우기
                        ...
                        nocut_arm->Fill(hitPerCube.at(key).trackLength);
                            ...
                            if(hitPerCube.at(key).energyDeposit > 0.000001)
                            {
                                cut_arm1->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000002)
                            {
                                cut_arm2->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000003)
                            {
                                cut_arm3->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000004)
                            {
                                cut_arm4->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000005)
                            {
                                cut_arm5->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000006)
                            {
                                cut_arm6->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000007)
                            {
                                cut_arm7->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000008)
                            {
                                cut_arm8->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.000009)
                            {
                                cut_arm9->Fill(hitPerCube.at(key).trackLength);
                            }
                            if(hitPerCube.at(key).energyDeposit > 0.00001)
                            {
...
void neutron(string filename)
{
    ...
    while (input >> file)
    {
        ...
        analyze(file);//
        filenum++;//진행상황 퍼센트 보려고 넣은거
    }//파일이 끝날 때까지 계속 넣기
    TH1D * entry = new TH1D("asdf","asdf",10,0,1);//더블 정밀도를 가진 1차원 히스토그램 선언. 빈 10, 처음0 끝1
	...
	for (int iLvr = 0; iLvr < 20; iLvr++)
	{
 		    ...
 		    threshold_vs_efficiency->SetBinContent(5,cut_arm4->GetEntries()/nocut_arm->GetEntries());
 	            ...
 	            entry->SetBinContent(1,cut_arm0->GetEntries());//엔트리라는 그래프의 1번째 빈에 애널라이즈로 얻은 cut_arm0를 넣기
      ...
      // while (purity > 0.9)
       //{threshold_vs_efficiency->SetBinContent(1,cut_arm0->GetEntries()/nocut_arm->GetEntries());
        ...
        //entry->SetBinContent(11,cut_arm10->GetEntries());
	//}
    	threshold_vs_efficiency->GetYaxis()->SetRangeUser(0,1);//이 그래프 세팅 바꾸기
    	threshold_vs_efficiency->GetYaxis()->SetTitle("efficiency");//이 그래프 세팅 바꾸기
   	threshold_vs_efficiency->GetXaxis()->SetTitle("threshold");//이 그래프 세팅 바꾸기
        ...
        //{
             ...
             //{
                 ...
                 // Fill histogram
                 //leverarm_vs_time_efficiency->SetBinContent(iLvr + 1, iTime + 1, cut_arm0->GetEntries()/nocut_arm->GetEntries());
             //}
        //}

	//leverarm_vs_time_efficiency->SetBinContent(1,cut_arm0->GetEntries());


    TFile * thre = new TFile("threshold_vs_eff.root","RECREATE");//히스토그램이 아니라 티파일을 만드는 것//열고
    entry->Write();//엔트리라는 변수로 저장된 그래프 그리라는 최종명령
    nocut_arm->Write();//노컷_암이라는 변수로 저장된 그래프 최종명령
    threshold_vs_efficiency->Write();//그래프 그리라는 최종명령
    leverarm_vs_time_efficiency->Write();//
    thre->Close();//만들어준 티파일 닫아주는 것//닫고
    ...
    cut_2D->Write();//cut_2D 그리라는 명령
    cut_time->Write();
    nocut_2D->Write();//nocut_2D 그리라는 최종 명령
    nocut_arm->Write();
    nocut_time->Write();
